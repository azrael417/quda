...
#include <dslash_init.cuh>
    ...
    void DiracMobiusDomainWallPC::M(ColorSpinorField &out, const ColorSpinorField &in) const
    {
        if(dagger == QUDA_DAG_NO)
        {
            ...
            //QUDA_MATPC_EVEN_EVEN : 1 - M5inv_{ee} * kappa_b^2 * D4_{eo}D4pre_{oe}D5inv_{ee}D4_{eo}D4pre_{oe}
            ...
            if (matpcType == QUDA_MATPC_EVEN_EVEN_ASYMMETRIC) {
                ...
                Dslash4(*tmp1, out, QUDA_ODD_PARITY);
                //invert M5
                Dslash5inv(out, *tmp1, QUDA_EVEN_PARITY, kappa5); //kappa5 is dummy value: kappa5 is set to kappa_b/kappa_c implicitly
                //add source:
                blas::xpay(const_cast<ColorSpinorField&>(in),1.,out);
        ...
        else
        {
            ...
            //QUDA_MATPC_EVEN_EVEN_ASYMMETRIC : M5 - kappa_b^2 * D4pre_{eo}D4_{oe}D5inv_{ee}D4pre_{eo}D4_{oe}
            //QUDA_MATPC_EVEN_EVEN : 1 - M5inv_{ee} * kappa_b^2 * D4_{eo}D4pre_{oe}D5inv_{ee}D4_{eo}D4pre_{oe}
            //QUDA_MATPC_ODD_ODD : M5 - kappa_b^2 * D4pre_{oe}D4_{eo}D5inv_{oo}D4pre_{oe}D4_{eo}
            //Actually, Dslash5 will return M5 operation and M5 = 1 + 0.5*kappa_b/kappa_c * D5
            if (matpcType == QUDA_MATPC_EVEN_EVEN_ASYMMETRIC) {
                ...
                Dslash4pre(*tmp1, out, QUDA_EVEN_PARITY);
                //invert M5
                Dslash5inv(out, *tmp1, QUDA_ODD_PARITY, kappa5); //kappa5 is dummy value: kappa5 is set to kappa_b/kappa_c implicitly
                //add source:
                blas::xpay(const_cast<ColorSpinorField&>(in),1.,out);
            } else if (matpcType == QUDA_MATPC_ODD_ODD_ASYMMETRIC) {
    ...
    const QudaSolutionType solType) const
    {
        ...
        if (solType == QUDA_MATPC_SOLUTION || solType == QUDA_MATPCDAG_MATPC_SOLUTION) {
            ...
            if (matpcType == QUDA_MATPC_EVEN_EVEN_ASYMMETRIC) {
                ...
                src = &(x.Odd());
                sol = &(x.Even());
            } else if (matpcType == QUDA_MATPC_EVEN_EVEN) { //we have to modify the source vector, since we multiply the asymmetric EE operator with M5inv from the left, which means that in the inverse, it is on the right and thus the source needs to be preconditioned by and additional factor of M5:
                // src = D5_{ee} * (b_e + k D4_eo*D5inv b_o)
                Dslash5inv(x.Odd(), b.Odd(), QUDA_ODD_PARITY, kappa5); //kappa5 is dummy
                Dslash4pre(*tmp1, x.Odd(), QUDA_ODD_PARITY);
                Dslash4Xpay(x.Odd(), *tmp1, QUDA_ODD_PARITY, b.Even(), 1.0);
                Dslash5(x.Even(), x.Even(), QUDA_EVEN_PARITY); //is that allowed?
                src = &(x.Odd());
                sol = &(x.Even());
            } else if (matpcType == QUDA_MATPC_ODD_ODD_ASYMMETRIC) {
    ...
    const QudaSolutionType solType) const
    {
        ...
        checkFullSpinor(x, b);
        //in the reconstructon step, we do not need to do anything different for EE compared to asym EE.
        if ( (matpcType == QUDA_MATPC_EVEN_EVEN_ASYMMETRIC) || (matpcType == QUDA_MATPC_EVEN_EVEN)  ) {
